...
// TODO: UIWebView is missing a header
                             ...
                             givenAction:(int)fp20
{
  ...
  if (!_ignoreInsertText) {
    const char* cmd_cstr =
      [character cStringUsingEncoding:[NSString defaultCStringEncoding]];
...
- (void)heartbeatCallback:(id)ignored
{
  ...
  while (1) {
    ...
    [view stopCapture];
    [[view _webView] insertText:(id)out];
    [view startCapture];
...
- (void) applicationDidFinishLaunching: (id) unused
{
    ...
    view = [[ShellView alloc]
               initWithFrame: CGRectMake(0.0f, 0.0f, 320.0f, 240.0f)];
    [view setText:@"Welcome!\n"];
    [view setTextSize:12];
	[view setTextColor:  CGColorCreate( colorSpace, white)];
    [view setTextFont:@"Courier"];
	[view setBackgroundColor: CGColorCreate( colorSpace, black)];
